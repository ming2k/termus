...
GENERIC_ITER_PREV(filters_get_prev, struct filter_entry, node)
GENERIC_ITER_NEXT(filters_get_next, struct filter_entry, node)

static int filters_view_get_sel(struct iter *iter)
{
	if (filters_view_ops && filters_view_ops->get_sel)
		return filters_view_ops->get_sel(filters_view_data, iter);
	return 0;
}

static void filters_view_set_sel(struct iter *iter)
{
	if (filters_view_ops && filters_view_ops->set_sel)
		filters_view_ops->set_sel(filters_view_data, iter);
}

static void filters_view_row_vanishes(struct iter *iter)
{
	if (filters_view_ops && filters_view_ops->row_vanishes)
		filters_view_ops->row_vanishes(filters_view_data, iter);
}

static void filters_view_mark_changed(void)
{
	if (filters_view_ops && filters_view_ops->mark_changed)
...
int filters_get_edit_command(char *buf, size_t size)
{
	...
	if (!filters_view_get_sel(&sel))
		return 0;

	e = iter_to_filter_entry(&sel);
	snprintf(buf, size, "fset %s=%s", e->name, e->filter);
	return 1;
}
