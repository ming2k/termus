...
{
	char *full = NULL;
	enum file_type type = termus_detect_ft(file, &full);
	int not_invalid = type != FILE_TYPE_INVALID;
	if (not_invalid)
		termus_add(pl_add_cb, full, type, JOB_TYPE_PL, 0, pl_marked);
	free(full);
	return not_invalid;
}
...
static void pl_load_one(const char *file)
{
	...
	termus_add(pl_add_cb, full, FILE_TYPE_PL, JOB_TYPE_PL, 0, pl);
...
pl_play_track(struct playlist *pl, struct simple_track *t, bool force_follow)
{
	...
	pl_playing_track = t;
	pl_playing = pl;
	editable_shared_mark_changed(&pl_editable_shared);

	if (force_follow || options_get_follow())
		pl_select_playing_track();
...
static void pl_save_one(struct playlist *pl)
{
	...
	termus_save(pl_save_cb, path, pl);
	free(path);
}
...
{
	if (yes_no_query("Delete selected playlist? [y/N]") != QUERY_ANSWER_YES)
		return;
...
void pl_import(const char *path)
{
	...
	termus_add(pl_add_cb, path, FILE_TYPE_PL, JOB_TYPE_PL, 0, pl);
	list_add_tail(&pl->node, &pl_head);
	pl_list_win->changed = 1;
...
{
	if (yes_no_query("Reload this playlist? [y/N]") != QUERY_ANSWER_YES)
		return;

	pl_clear_visible_pl();

	char *full = pl_name_to_pl_file(pl_visible->name);
	termus_add(pl_add_cb, full, FILE_TYPE_PL, JOB_TYPE_PL, 0, pl_visible);
	free(full);
}
...
void pl_set_nr_rows(int h)
{
	...
	editable_shared_set_nr_rows(&pl_editable_shared, h);
}
	     ...
	     void (*tracks)(struct window *win), int full)
{
	if (full || pl_list_win->changed)
		list(pl_list_win);
	if (full || editable_shared_needs_redraw(&pl_editable_shared))
		tracks(pl_track_win());
