...
static pthread_mutex_t job_mutex = TERMUS_MUTEX_INITIALIZER;
...
#define job_lock() termus_mutex_lock(&job_mutex)
#define job_unlock() termus_mutex_unlock(&job_mutex)
...
static void add_pl(const char *filename)
{
	...
	if (buf) {
		...
		termus_playlist_for_each(buf, size, reverse, handle_line, cwd);
		free(cwd);
		munmap(buf, size);
