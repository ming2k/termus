		...
		int show_total_time, int show_sort_keys)
{
	...
	if (show_sort_keys && e->shared->sort_str[0] != 0)
		gbuf_addf(&buf, " sorted by %s", e->shared->sort_str);

	update_window(win, 0, 0, win_w, buf.buffer, &print_editable);
}
...
static void do_update_statusline(void)
{
	...
	int progress_bar = options_get_progress_bar();
...
static void ui_info_msg(const char *format, ...)
{
	va_list ap;
...
static void ui_error_msg(const char *format, ...)
{
	va_list ap;
...
static int ui_yes_no_query(const char *format, ...)
{
	static GBUF(buffer);
	gbuf_clear(&buffer);
	va_list ap;
...
static void write_status_file(const char *status_text, struct track_info *ti)
{
	const char *path = options_get_status_display_file();
	if (!path || !path[0])
		return;
...
{
	const char *status_text = player_status_names[player_info.status];
	write_status_file(status_text, player_info.ti);
	spawn_status_program_inner(status_text, player_info.ti);
}
...
static void handle_ch(uchar ch)
{
	clear_error();
	if (input_mode == NORMAL_MODE) {
		if (!options_get_block_key_paste() || !in_bracketed_paste) {
			normal_mode_ch(ch);
		}
...
#define SELECT_ADD_FD(fd) do {\
	...
	while (termus_running) {
		...
		SELECT_ADD_FD(termus_next_track_request_fd);
		...
		for (i = 0; i < nr_fds_out; i++) {
			if (FD_ISSET(fds_out[i], &set)) {
				d_print("out changed\n");
				if (options_get_pause_on_output_change()) {
		...
		if (FD_ISSET(termus_next_track_request_fd, &set))
			termus_provide_next_track();
	}
}
...
static void init_all(void)
{
	...
	lib_autosave_filename = xstrjoin(termus_user_data_dir, "/lib.pl");
	play_queue_autosave_filename = xstrjoin(termus_state_dir, "/queue.pl");
...
int main(int argc, char *argv[])
{
	...
	program_name = argv[0];
	info_handler = ui_info_msg;
	error_handler = ui_error_msg;
	yes_no_query_handler = ui_yes_no_query;
	argv++;
	while (1) {
		int idx;
		char *arg;
	...
	const char *detected_charset = getenv("TERMUS_CHARSET");
	if (!detected_charset || !detected_charset[0]) {
#ifdef CODESET
		detected_charset = nl_langinfo(CODESET);
#else
		detected_charset = "ISO-8859-1";
#endif
	}
	locale_init_charset(detected_charset);

	misc_init();
	if (server_address == NULL)
		server_address = xstrdup(termus_socket_path);
	debug_init();
	d_print("charset = '%s'\n", charset);
