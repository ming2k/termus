...
void view_add(int view, char *arg, int prepend)
{
	...
	case QUEUE_VIEW:
		if (prepend) {
			...
			termus_add(play_queue_append, name, ft, JOB_TYPE_QUEUE, 0,
					NULL);
		}
		break;
	default:
...
{
	info_msg("To get started with termus, read termus-tutorial(7) and termus(1) man pages");
}
...
static void cmd_quit(char *arg)
{
	...
	if (!worker_has_job_by_type(JOB_TYPE_ANY)) {
		if (flag != 'i' || yes_no_query("Quit termus? [y/N]") != QUERY_ANSWER_NO)
			termus_running = 0;
	} else {
...
static void cmd_run(char *arg)
{
	...
	run = 1;
	if (options_get_confirm_run() && (sel.tis_nr > 1 || strcmp(argv[0], "rm") == 0)) {
		if (yes_no_query("Execute %s for the %d selected files? [y/N]", arg, sel.tis_nr) != QUERY_ANSWER_YES) {
			info_msg("Aborted");
			run = 0;
		}
	}
