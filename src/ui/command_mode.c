...
void view_add(int view, char *arg, int prepend)
{
	...
	case QUEUE_VIEW:
		if (prepend) {
			termus_add(play_queue_prepend, name, ft, JOB_TYPE_QUEUE,
				   0, NULL);
		} else {
			termus_add(play_queue_append, name, ft, JOB_TYPE_QUEUE,
				   0, NULL);
		}
		break;
	default:
...
void view_save(int view, char *arg, int to_stdout, int filtered, int extended)
{
	...
	case QUEUE_VIEW:
		if (worker_has_job_by_type(JOB_TYPE_QUEUE))
			goto worker_running;
		dest =
		    extended ? &play_queue_ext_filename : &play_queue_filename;
...
{
	struct termus_opt *opt = option_find(arg);
...
static void cmd_speed(char *arg)
{
	...
	val = strtod(arg, &end);
	if (end == arg || *end != '\0') {
		error_msg("expecting: [+-]SPEED (e.g. 1.0, +0.1, -0.5)");
		return;
	}

	if (relative) {
		val = playback_speed + relative * val;
	}
...
static void cmd_factivate(char *arg) { filters_activate_names(arg); }
...
static void cmd_filter(char *arg) { filters_set_anonymous(arg); }

static void cmd_fset(char *arg) { filters_set_filter(arg); }

static void cmd_help(char *arg)
{
	info_msg("To get started with termus, read termus-tutorial(7) and "
		 "termus(1) man pages");
}
...
static void cmd_quit(char *arg)
{
	...
	if (!worker_has_job_by_type(JOB_TYPE_ANY)) {
		if (flag != 'i' ||
		    yes_no_query("Quit termus? [y/N]") != QUERY_ANSWER_NO)
			termus_running = 0;
	} else {
		answer = yes_no_query("Tracks are being added. Quit and "
				      "truncate playlist(s)? [y/N]");
		if (answer != QUERY_ANSWER_NO)
			termus_running = 0;
	}
}
...
{
	if (options_display_apply_colorscheme(arg) == -1)
		error_msg("loading colorscheme %s: %s", arg, strerror(errno));
}
...
static void cmd_run(char *arg)
{
	...
	run = 1;
	if (options_get_confirm_run() &&
	    (sel.tis_nr > 1 || strcmp(argv[0], "rm") == 0)) {
		if (yes_no_query("Execute %s for the %d selected files? [y/N]",
				 arg, sel.tis_nr) != QUERY_ANSWER_YES) {
			info_msg("Aborted");
			run = 0;
		}
	}
	if (run) {
		...
		if (spawn(argv, &status, 1)) {
			...
			case SORTED_VIEW:
				...
				/* remove non-existed files, update tags for
				 * changed files */
				termus_update_tis(sel.tis, sel.tis_nr, 0);
...
err:
	free_str_array(values);
	error_msg("expecting 1 or 2 arguments (total or L and R volumes "
		  "[+-]INTEGER[%%])\n");
}
...
static void cmd_version(char *arg) { info_msg(VERSION); }
...
static void cmd_p_next(char *arg) { termus_next(); }

static void cmd_p_pause(char *arg) { player_pause(); }

static void cmd_p_pause_playback(char *arg) { player_pause_playback(); }

static void cmd_p_play(char *arg)
{
	if (arg) {
		char *tmp = expand_filename(arg);
		termus_play_file(tmp);
		free(tmp);
	} else {
		player_play();
	}
...
static void cmd_p_next_album(char *arg) { termus_next_album(); }

static void cmd_p_prev_album(char *arg) { termus_prev_album(); }

static void cmd_p_stop(char *arg) { player_stop(); }
...
static void cmd_raise_vte(char *arg) { termus_raise_vte(); }
...
static void cmd_search_start(char *arg) { enter_search_mode(); }

static void cmd_search_b_start(char *arg) { enter_search_backward_mode(); }
...
static void cmd_win_activate(char *arg)
{
	...
	if (info) {
		if (options_get_shuffle() && next)
			shuffle_insert(shuffle_root, previous, next);
		/* update lib/pl mode */
		if (cur_view < 2)
			options_set_play_library(1);
		if (cur_view == 2)
			options_set_play_library(0);
...
static void cmd_win_scroll_up(char *arg) { window_scroll_up(current_win()); }

static void cmd_win_bottom(char *arg) { window_goto_bottom(current_win()); }
...
static void cmd_win_pg_down(char *arg) { window_page_down(current_win()); }

static void cmd_win_pg_up(char *arg) { window_page_up(current_win()); }
...
static void cmd_win_hf_pg_up(char *arg) { window_half_page_up(current_win()); }

static void cmd_win_pg_top(char *arg) { window_page_top(current_win()); }

static void cmd_win_pg_bottom(char *arg) { window_page_bottom(current_win()); }

static void cmd_win_pg_middle(char *arg) { window_page_middle(current_win()); }
...
static void cmd_win_update_cache(char *arg)
{
	...
	if (sel.tis_nr == 0)
		return;
	sel.tis[sel.tis_nr] = NULL;
	termus_update_tis(sel.tis, sel.tis_nr, flag == 'f');
}

static void cmd_win_top(char *arg) { window_goto_top(current_win()); }
...
static int filter_any(const char *name, const struct stat *s) { return 1; }

static int filter_playable(const char *name, const struct stat *s)
{
	return S_ISDIR(s->st_mode) || termus_is_playable(name);
}

static int filter_playlist(const char *name, const struct stat *s)
{
	return S_ISDIR(s->st_mode) || termus_is_playlist(name);
}

static int filter_supported(const char *name, const struct stat *s)
{
	return S_ISDIR(s->st_mode) || termus_is_supported(name);
}
...
static void expand_colorscheme(const char *str)
{
	...
	load_themes(termus_config_dir, str, &array);
	load_themes(termus_data_dir, str, &array);
		...
		if (strncmp(cmd, c->name, cmd_len) == 0) {
			...
			if (run_only_safe_commands && (c->flags & CMD_UNSAFE)) {
				if (c->func != cmd_save ||
					...
					d_print("trying to execute unsafe "
						"command over net\n");
					break;
				}
			}
...
{
	cmd_history_filename = xstrjoin(termus_state_dir, "/command-history");
