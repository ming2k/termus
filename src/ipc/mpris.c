		...
		sd_bus_error *_ret_error)
{
	...
	CK(sd_bus_message_read_basic(value, 'd', &rate));
	if (rate < 0.1)
		rate = 0.1;
	else if (rate > 4.0)
		rate = 4.0;

	player_set_speed(rate);
	options_hooks_refresh_statusline();
	return sd_bus_reply_method_return(value, "");
}
