...
static void generic_set_int(void *data, const char *buf)
{
	...
	if (str_to_int(buf, &tmp) == -1 || tmp < opt->extra.i.min || tmp > opt->extra.i.max) {
		error_msg("integer in range %d..%d expected", opt->extra.i.min, opt->extra.i.max);
		return;
	}
	*val = (int)tmp;
	if (opt->post_set)
...
static void option_insert(struct termus_opt *opt)
{
	...
	while (item != &option_head) {
		struct termus_opt *o = container_of(item, struct termus_opt, node);
...
void option_add_int_full(const char *name, int *val, int min, int max, opt_post_set_cb post_set, unsigned int flags)
{
	struct termus_opt *opt = xnew(struct termus_opt, 1);
...
void option_add_int(const char *name, int *val, int min, int max, unsigned int flags)
{
	option_add_int_full(name, val, min, max, NULL, flags);
}

void option_add_bool_full(const char *name, int *val, opt_post_set_cb post_set, unsigned int flags)
{
	struct termus_opt *opt = xnew(struct termus_opt, 1);
...
void option_add_str_full(const char *name, char **val, opt_post_set_cb post_set, unsigned int flags)
{
	struct termus_opt *opt = xnew(struct termus_opt, 1);
