#ifndef TERMUS_CHANNELMAP_H
#define TERMUS_CHANNELMAP_H
...
static inline int channel_map_equal(const channel_position_t *a,
				    const channel_position_t *b, int channels)
{
	return memcmp(a, b, sizeof(*a) * channels) == 0;
}

static inline channel_position_t *
channel_map_copy(channel_position_t *dst, const channel_position_t *src)
{
...
void channel_map_init_waveex(int channels, unsigned int mask,
			     channel_position_t *map);
