#ifndef TERMUS_TRACK_INFO_H
#define TERMUS_TRACK_INFO_H
...
#define REV_SORT_ARTIST                                                        \
	(REV_SORT__START + offsetof(struct track_info, collkey_artist))
#define REV_SORT_ALBUM                                                         \
	(REV_SORT__START + offsetof(struct track_info, collkey_album))
#define REV_SORT_TITLE                                                         \
	(REV_SORT__START + offsetof(struct track_info, collkey_title))
#define REV_SORT_DURATION                                                      \
	(REV_SORT__START + offsetof(struct track_info, duration))
#define REV_SORT_PLAY_COUNT                                                    \
	(REV_SORT__START + offsetof(struct track_info, play_count))
#define REV_SORT_TRACKNUMBER                                                   \
	(REV_SORT__START + offsetof(struct track_info, tracknumber))
#define REV_SORT_DISCNUMBER                                                    \
	(REV_SORT__START + offsetof(struct track_info, discnumber))
#define REV_SORT_TOTALDISCS                                                    \
	(REV_SORT__START + offsetof(struct track_info, totaldiscs))
#define REV_SORT_DATE (REV_SORT__START + offsetof(struct track_info, date))
#define REV_SORT_ORIGINALDATE                                                  \
	(REV_SORT__START + offsetof(struct track_info, originaldate))
#define REV_SORT_RG_TRACK_GAIN                                                 \
	(REV_SORT__START + offsetof(struct track_info, rg_track_gain))
#define REV_SORT_RG_TRACK_PEAK                                                 \
	(REV_SORT__START + offsetof(struct track_info, rg_track_peak))
#define REV_SORT_RG_ALBUM_GAIN                                                 \
	(REV_SORT__START + offsetof(struct track_info, rg_album_gain))
#define REV_SORT_RG_ALBUM_PEAK                                                 \
	(REV_SORT__START + offsetof(struct track_info, rg_album_peak))
#define REV_SORT_GENRE                                                         \
	(REV_SORT__START + offsetof(struct track_info, collkey_genre))
#define REV_SORT_COMMENT                                                       \
	(REV_SORT__START + offsetof(struct track_info, collkey_comment))
#define REV_SORT_ALBUMARTIST                                                   \
	(REV_SORT__START + offsetof(struct track_info, collkey_albumartist))
#define REV_SORT_FILENAME                                                      \
	(REV_SORT__START + offsetof(struct track_info, filename))
#define REV_SORT_FILEMTIME                                                     \
	(REV_SORT__START + offsetof(struct track_info, mtime))
#define REV_SORT_BITRATE                                                       \
	(REV_SORT__START + offsetof(struct track_info, bitrate))
#define REV_SORT_CODEC (REV_SORT__START + offsetof(struct track_info, codec))
#define REV_SORT_CODEC_PROFILE                                                 \
	(REV_SORT__START + offsetof(struct track_info, codec_profile))
 ...
 *
 * returns: 1 if all words in @text are found to match defined fields (@flags)
 * in @ti 0 otherwise
 */
int track_info_matches(const struct track_info *ti, const char *text,
		       unsigned int flags);
 ...
 *
 * returns: 1 if all/any words in @text are found to match defined fields
 * (@flags) in @ti 0 otherwise
 */
int track_info_matches_full(const struct track_info *ti, const char *text,
			    unsigned int flags, unsigned int exclude_flags,
			    int match_all_words);

int track_info_cmp(const struct track_info *a, const struct track_info *b,
