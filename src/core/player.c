...
static pthread_mutex_t producer_mutex = TERMUS_MUTEX_INITIALIZER;
static pthread_cond_t producer_playing = TERMUS_COND_INITIALIZER;
...
static pthread_mutex_t consumer_mutex = TERMUS_MUTEX_INITIALIZER;
static pthread_cond_t consumer_playing = TERMUS_COND_INITIALIZER;
...
#define player_info_priv_lock() termus_mutex_lock(&player_info_mutex)
#define player_info_priv_unlock() termus_mutex_unlock(&player_info_mutex)

#define producer_lock() termus_mutex_lock(&producer_mutex)
#define producer_unlock() termus_mutex_unlock(&producer_mutex)
...
/* Return the sample format to use for the output plugin, which may differ
 * from buffer_sf when playback_speed != 1.0.  Changing the hardware rate
 * while keeping the buffer at the source rate produces a tape-speed effect:
 * the hardware drains the buffer faster/slower, playing content at the
 * requested speed while pitch shifts proportionally.
 */
static sample_format_t get_output_sf(void)
{
...
static void TERMUS_FORMAT(2, 3) player_ip_error(int rc, const char *format, ...)
{
	char buffer[1024];
	va_list ap;
	char *msg;
...
static void TERMUS_FORMAT(2, 3) player_op_error(int rc, const char *format, ...)
{
	char buffer[1024];
	va_list ap;
	char *msg;
...
static void _producer_play(void)
{
	if (producer_status == PS_UNLOADED) {
		...
		rc = ip_open(ip);
		if (rc) {
			const char *fn = ip_get_filename(ip);
...
static void _consumer_play(void)
{
	if (consumer_status == CS_PLAYING) {
		...
		set_buffer_sf();
		rc = op_open(get_output_sf(), buffer_channel_map);
		if (rc) {
...
void player_info_snapshot(void)
{
	...
	player_info_priv.speed_changed = 0;
	player_info_priv.error_msg = NULL;

	player_info_priv_unlock();
}

static void player_reopen_complete(int was_paused)
{
	if (was_paused) {
		op_pause();
		_consumer_status_update(CS_PAUSED);
...
void player_set_speed(double speed)
{
	...
	player_lock();
	if (fabs(playback_speed - speed) < 0.000001) {
		player_unlock();
		return;
	}
	...
	if (consumer_status == CS_PLAYING || consumer_status == CS_PAUSED) {
		int was_paused = (consumer_status == CS_PAUSED);
		int rc;
