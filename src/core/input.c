...
static pthread_rwlock_t ip_lock = TERMUS_RWLOCK_INITIALIZER;

#define ip_rdlock() termus_rwlock_rdlock(&ip_lock)
#define ip_wrlock() termus_rwlock_wrlock(&ip_lock)
#define ip_unlock() termus_rwlock_unlock(&ip_lock)
...
static int read_playlist(struct input_plugin *ip, int sock)
{
	...
	termus_playlist_for_each(body, size, 0, handle_line, &rpd);
	free(body);
	if (!rpd.count) {
