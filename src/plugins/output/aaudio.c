...
// mapping from AAUDIO_CHANNEL_* enum values to termus channel_position_t values
//
...
// mapping from AAUDIO_CHANNEL_* masks to termus channel_position_t lists
//
...
// convert a termus channel map to an equivalent aaudio channel mask (the returned
// value will either be invalid or have the same number of bits set as the
// number of channels)
static aaudio_channel_mask_t termus_channel_map_to_aaudio_mask(int channels, const channel_position_t *channel_map)
{
	...
	// special case for mono since termus defines a separate channel position
	// for it
...
// get the expected termus channel order for the specified aaudio channel mask
...
static aaudio_result_t configure_aaudio_sf(AAudioStreamBuilder *builder, sample_format_t sf, const channel_position_t *channel_map, ssize_t **out_remap)
{
	...
	*out_remap = NULL;
	if (channel_map && channel_map_valid(channel_map)) {
		if (API_AT_LEAST(32)) {
			mask = termus_channel_map_to_aaudio_mask(sf_get_channels(sf), channel_map);
			...
			if (mask == AAUDIO_CHANNEL_INVALID) {
				...
				if (!channel_map_init_aaudio(mask, mask_expected_channels)) {
					d_print("not applying channel map since there isn't a valid termus channel mapping for the aaudio mask\n");
				} else {
...
static int op_aaudio_open(sample_format_t sf, const channel_position_t *channel_map)
{
	...
	if (API_AT_LEAST(31)) AAudioStreamBuilder_setAttributionTag(bld, "termus");
...
static int op_aaudio_write(const char *buf, int count)
{
	...
	// if the stream errored, return an error so termus restarts the output
	// plugin
	//
	// note that this isn't strictly required since AAudioStream_write will
	// return an error on stream disconnection, which will cause termus to
	...
	// flushing since termus may not always use the stream and starting a
	...
	// be less than AAudioStream_getBufferCapacityInFrames, and termus
