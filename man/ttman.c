...
#else
#define TERMUS_NORETURN
#endif

static TERMUS_NORETURN void quit(void)
{
	if (tmp_file[0])
		unlink(tmp_file);
	exit(1);
...
static TERMUS_NORETURN void die(const char *format, ...)
{
	va_list ap;
...
static TERMUS_NORETURN void syntax(int line, const char *format, ...)
{
	va_list ap;
